...
public class SC_ItemCrafting : MonoBehaviour
{
    ...
    void InitializeSlotTable(RectTransform container, SC_SlotTemplate slotTemplateTmp, SlotContainer[] slots, int margin, int tableIDTmp)
    {
        ...
        for (int i = 0; i < slots.Length; i++)
        {
            ...
            newSlot.SetActive(true);  // Make sure the instantiated object is active
    ...
    public void ClickEventRecheck()
    {
        if (selectedItemSlot == null)
        {
            ...
            if (selectedItemSlot != null)
            {
                ...
                SlotContainer clickedSlot = GetClickedSlot();
                if (clickedSlot != null && clickedSlot.tableID == resultTableID) // Ensuring it's the result slot
                {
                    // Load the NPC (Tiara) scene if the result slot is clicked and has an item
                    if (clickedSlot.itemSprite != null)
                    {
                        UnityEngine.SceneManagement.SceneManager.LoadScene("NPC(Tiara)");
                    }
                }
    ...
    public void AddItemToCraftingTable(SlotContainer inventorySlot)
    {
        Debug.Log("Attempting to add item to crafting table: " + inventorySlot.itemSprite.name);

        // Suppose you have a method to find an empty slot in the crafting table
        SlotContainer emptyCraftingSlot = FindEmptyCraftingSlot();
        if (emptyCraftingSlot != null)
        {
            emptyCraftingSlot.itemSprite = inventorySlot.itemSprite;
            emptyCraftingSlot.itemCount = inventorySlot.itemCount;
            Debug.Log("Added " + inventorySlot.itemCount + " " + inventorySlot.itemSprite.name + " to crafting table.");
            UpdateItems(craftSlots); // Update the UI if necessary
        }
        else
        {
            Debug.Log("No empty slot available in crafting table.");
        }
    }

    private SlotContainer FindEmptyCraftingSlot()
    {
    ...
    void OnEnable()
    {
        Debug.Log("Script enabled on " + gameObject.name);
       
        Debug.Log(gameObject.name + " enabled: " + gameObject.activeSelf);
